...
结果页把《开源及第三方资源使用清单》**作为首要输出**：11 列表格直接显示在页面上，
尚未采集到源码证据的单元格标黄为「待确认」并给出计数，同时提示上传 zip 可自动判定。

清单有三种出口——**页面表格 / Markdown / CSV**——全部由 `to_checklist_rows()` 一处渲染，
**内容逐格一致**（此前 CSV 由前端另拼一张 8 列表，下载到的并不是竞赛要求的那份清单，
页面上的版本也与两者都对不上）。CSV 带 BOM 与 `\r\n`，用 Excel 直接打开不乱码；
Markdown 可直接粘进报告正文。三者的一致性由 Q 组用例守护。
...
```bash
python test_cases.py      # 277 个用例：许可证归一化 + 兼容性判定 + 矩阵覆盖 + 版本约束 + 清单表 + 抽查抓取状态
...
共 **372 个用例，全部可离线运行**。**每个用例都对应开发过程中实测发现的真实误判，不是编造的假数据**——
